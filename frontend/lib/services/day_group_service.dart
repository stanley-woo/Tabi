...
class DayGroupService {
  ...
  static Future<DayGroup> createDayGroup({required int itineraryId, required DateTime date, String? title}) async {
    final uri = Uri.parse('$_base/itineraries/$itineraryId/days/');
    final payload = {'date': date.toIso8601String().substring(0,10), 'order': 0, 'title': title};
  ...
  /// Delete a day group by its ID.
  static Future<void> deleteDayGroup(int itineraryId, int dayGroupId) async {
    final uri = Uri.parse('$_base/itineraries/$itineraryId/days/$dayGroupId');
    final resp = await http.delete(uri);
    if (resp.statusCode == 404) {
      throw Exception('Day group not found');
    }
  ...
  /// Reorder day groups. Supply the new sequence of IDs.
  static Future<List<DayGroup>> reorderDayGroups(
    int itineraryId, List<int> orderedIds) async {
    final uri = Uri.parse('$_base/itineraries/$itineraryId/days/reorder');
    final resp = await http.patch(uri, headers: {'Content-Type': 'application/json'}, body: jsonEncode(orderedIds));
    if (resp.statusCode != 200) {
